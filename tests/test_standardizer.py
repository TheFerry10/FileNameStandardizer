...
def test_extract_datetime_string_android_file() -> None:
    ...
    datetime_string = standardizer.extract_datetime_string(file_name, datetime_pattern)
...
def test_get_datetime(
    ...
    datetime_ = standardizer.get_datetime(datetime_string, format_)
...
def test_extract_index() -> None:
    ...
    file_name_index = standardizer.extract_index(file_name, index_pattern)
...
def test_file_name_standardization_with_schema(
    ...
    file_name_schema: standardizer.FileNameSchema,
    expected_target_file_name: str,
) -> None:

    target_file_name = standardizer.standardize_file_name(
...
def test_file_name_standardization_with_schema_and_identifier() -> None:
    ...
    file_name_schema = standardizer.ANDROID_SCHEMA
    identifier = "Device0"
...
def test_standardize_file_name_with_schema_estimation(
    ...
    target_file_name = standardizer.standardize_file_name(original_file_name)
    assert expected_target_file_name == str(target_file_name)
...
def test_standardize_file_name_raise_value_error_on_unknown_file_schema() -> None:
    ...
    with pytest.raises(ValueError):
        standardizer.standardize_file_name(original_file_name)


@pytest.mark.parametrize(
    "file_name, expected_file_name_schema",
    [
        ("IMG-20240721-WA0007.jpg", standardizer.WHATSAPP_SCHEMA),
        ("VID-20240721-WA0007.mp4", standardizer.WHATSAPP_SCHEMA),
        ("20240721_121233.jpg", standardizer.ANDROID_SCHEMA),
        ("20240721_121233.mp4", standardizer.ANDROID_SCHEMA),
    ],
)
def test_identify_file_schema_from_filename(
    file_name: str, expected_file_name_schema: standardizer.FileNameSchema
) -> None:
    file_name_schema = standardizer.identify_file_pattern(file_name)
...
def test_raise_value_error_on_unknown_file_schema() -> None:
    ...
    with pytest.raises(ValueError):
        standardizer.identify_file_pattern(file_name)
